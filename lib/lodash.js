...
var Lodash = {
  ...
  countBy: function (collection, identity) {
    ...
    identity = identity || function (item) {return item;};
    var result = {};
  ...
  every: function (collection, identity) {
    identity = identity || function (item) {return item;};
    return !!collection.reduce(function (mem, item) {
      return mem && identity(item);
    }, true);
  },
  ...
  find: function (collection, callback) {
    return collection.reduce(function (mem, item) {
      return callback(item) && mem.push(item) && mem || mem;
    }, []);
  },

  findLast: function (collection, callback) {
    var self = this;
    return self.find(collection.slice().reverse(), callback);
  },
  ...
  invoke: function (collections, identity) {
    var self = this;
    var args = self.toArray(arguments);
    var identityArgs = args.slice(2);
    if (typeof identity === 'string') {
      identity = collections[0][identity];
    }
    return collections.map(function (collection) {
      return identity.apply(collection, identityArgs);
    });
  },
  ...
  curry: function (fn) {
    ...
    return (function _curry() {
      var oldArgs = [];
      return (function __curry() {
        var args = self.toArray(arguments);
        oldArgs = oldArgs.concat(args);
        if (oldArgs.length < arity) {
          return __curry;
        } else {
          return fn.apply(this, oldArgs);
        }
  ...
  mapValues: function (obj, callback) {
    var self = this;
    callback = callback || self.identity;
    var newHash = {};
    for (var k in obj) {
      newHash[k] = callback(obj[k]);
    }
    return newHash;
  },
